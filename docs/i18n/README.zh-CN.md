...
- 新增 **清理过去的 Daily Note**，用于预览并应用历史生成行清理。
- 当 Todoist 任务仍未完成但已经不再属于那篇 Daily Note 日期时，可移除历史未完成行。
- 当 Todoist 确认任务已经完成时，可把历史行标记为完成。
- 也可以选择移除已完成的历史行，适合希望过去 Daily Note 更干净的用户。
- 普通同步逻辑保持不变：除非用户主动运行清理工具，过去 Daily Note 仍作为历史快照保留。
