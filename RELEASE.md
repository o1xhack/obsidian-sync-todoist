...
- `0.6.2`: Daily Note recurring completion and community review cleanup release. Adds a nested "Include completed recurring tasks" option under "Include completed tasks", uses Todoist activity logs to keep completed recurring occurrences in today's Daily Note, prevents generated Daily Note rows and ordinary imported task rows from downgrading timed or recurring Todoist due rules during bidirectional sync, removes Obsidian 1.8.7-only local storage API usage, replaces `builtin-modules` with Node's `builtinModules`, clears unsafe `any` settings persistence warnings, and rewrites the README structure.
- `0.7.0`: structured due, time, and recurring safety release. Adds a structured due model for Todoist all-day dates, floating times, fixed times, and recurring occurrences; supports Markdown floating-time syntax; sends editable local times with Todoist `due_datetime`; preserves fixed-time and recurring metadata with hidden `todoist-due` comments; prevents unsafe due-rule downgrades; fixes completed recurring Daily Note recovery from Todoist activity logs; preserves recurring completions made during the current sync cycle while Todoist Activity Log is delayed; preserves Obsidian wikilink headings such as `[[note#heading]]` instead of treating the anchor as a Todoist label; shows version/build information in General settings; updates import, query block, and Daily Note due display; and expands unit/API/manual test coverage.
- `0.8.0`: Daily Note completed-task mode and update-notes release. Replaces the single completed-task toggle with a three-mode selector for hiding completed tasks, keeping completed tasks due today, or keeping all tasks completed today; migrates the old enabled toggle to the previous completed-today behavior; keeps completed recurring occurrence recovery as a separate option; improves settings tab highlighting on mobile; adds a clickable version row with bilingual update notes that opens after an update; and starts `CHANGELOG.md` as the ongoing release history.
- `0.9.0`: past Daily Note cleanup release. Adds a manual preview-and-apply cleanup tool for Sync Todoist marker regions in past Daily Notes; removes unfinished historical rows whose active Todoist task no longer belongs to that date; marks historical rows completed when Todoist confirms completion; optionally removes completed historical rows; and keeps normal sync behavior unchanged unless the user runs cleanup.
