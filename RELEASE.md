...
- `0.7.0`: structured due, time, and recurring safety release. Adds a structured due model for Todoist all-day dates, floating times, fixed times, and recurring occurrences; supports Markdown floating-time syntax; sends editable local times with Todoist `due_datetime`; preserves fixed-time and recurring metadata with hidden `todoist-due` comments; prevents unsafe due-rule downgrades; fixes completed recurring Daily Note recovery from Todoist activity logs; updates import, query block, and Daily Note due display; and expands unit/API/manual test coverage.
